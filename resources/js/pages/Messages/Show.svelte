    ...
    <section class="space-y-8">
        <div class="theme-panel rounded-2xl p-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                    ...
                    <div class="mt-3 text-sm text-primary">{t('inbox.with_label')} {message.buyer}</div>
                    {#if message.createdAt}
                        <div class="mt-1 text-xs uppercase tracking-[0.14em] text-muted-foreground">{message.createdAt}</div>
                    {/if}
                </div>
                <div class="flex items-center gap-3">
                    <Form action={`/messages/${message.id}/close`} method="post">
                        {#snippet children({ processing })}
                            <Button type="submit" variant="outline" disabled={processing}>{t('inbox.close_conversation')}</Button>
                        {/snippet}
                    </Form>
                    <Link href="/messages/inbox" class="theme-link text-sm">
        ...
        <div class="grid gap-6 xl:grid-cols-[0.78fr_1.22fr]">
            <div class="space-y-6">
                <div class="theme-panel rounded-2xl p-6">
                    ...
                    <div class="mt-5 space-y-4 text-sm text-muted-foreground">
                        <div>
                            <div class="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{t('inbox.with_label')}</div>
            ...
            <div class="space-y-6">
                ...
                <div class="theme-panel rounded-2xl p-6">
                    <h2 class="text-lg font-semibold text-foreground">{t('inbox.reply_title')}</h2>
                    <p class="mt-2 text-sm text-muted-foreground">{t('inbox.reply_text')}</p>

                    <Form action={`/messages/${message.id}/reply`} method="post" class="mt-5 space-y-4">
                        {#snippet children({ errors, processing, recentlySuccessful })}
                            ...
                            <Button type="submit" disabled={processing}>{t('inbox.send_reply')}</Button>
                        {/snippet}
                    </Form>
                </div>
            </div>
